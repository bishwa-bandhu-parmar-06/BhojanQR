...
// Check if a table number is valid based on generated QR codes
exports.validateTableNumber = asyncHandler(async (req, res, next) => {
  const { id } = req.params; // restaurantId
  const { table } = req.query; // table number from URL query

  if (!table) {
    return next(new ErrorResponse("Table number is required", 400));
  }

  // Find if this specific table's QR was generated by the owner
  ...
  if (!existingQR) {
    return res.status(200).json({
      success: true,
      isValid: false,
      message: "Invalid Table! This table QR code does not exist or has been deleted.",
    });
  }
